...
def create_account():
    """
    Create account without email OTP verification
...
def login():
    ...
    phone = normalize_phone(data.get('phone', data.get('mobile', '')))

    if name or phone:
        if not name or len(phone) != 10:
            return error_response('Name and 10-digit mobile number are required', 400)
        ...
        try:
            ...
            if not user:
                generated_email = f'mobile_{phone}@restom.local'
                suffix = 1
                while User.query.filter_by(email=generated_email).first():
                    generated_email = f'mobile_{phone}_{suffix}@restom.local'
                    suffix += 1
                ...
                if user.role != 'customer':
                    return error_response('Please use email and password for this account', 403)
