...
async function updateAdminProfile(event) {
  ...
  if (!name) {
    showToast("errorToast", "Please enter your name");
    alert("Please enter your name");
    return;
  }

  if (mobile && !/^\d{10}$/.test(mobile)) {
    showToast("errorToast", "Mobile number must be exactly 10 digits");
  ...
  if (newPassword || confirmPassword) {
    if (!currentPassword) {
      showToast("errorToast", "Please enter your current password to change password");
      alert("Please enter your current password to change password");
      return;
    }

    if (newPassword.length < 6) {
      showToast("errorToast", "New password must be at least 6 characters");
      alert("New password must be at least 6 characters");
      return;
    }

    if (newPassword !== confirmPassword) {
      showToast("errorToast", "New password and confirm password do not match");
  ...
  try {
    ...
    if (response && response.success) {
      ...
      if (user) {
        ...
        localStorage.setItem("restom_user", JSON.stringify(user));
      }
      ...
      showToast("errorToast", response?.message || "Failed to update profile");
